...
protected function imageUrl(): Attribute
{
    return Attribute::make(
        get: function () {
            ...
            // Use s3 when configured, otherwise always the public disk so
            // Storage::url() prepends APP_URL and generates an absolute URL.
            $disk = config('app.media_disk') === 's3' ? 's3' : 'public';
...
> **Important**: Do NOT use `config('filesystems.default')` to determine the disk — it returns `'local'` in most configs, and `Storage::disk('local')->url()` produces relative paths (no origin). Always use `'public'` for local/public storage so the disk's configured `url` option (`APP_URL/storage`) is used.

In the Resource class, delegate to the model accessor rather than recomputing:

```php
// ❌ Wrong — recomputes with the wrong disk
'image_url' => $this->image_path ? Storage::disk(config('app.media_disk', config('filesystems.default')))->url($this->image_path) : null,

// ✅ Correct — uses the accessor which has the right logic
