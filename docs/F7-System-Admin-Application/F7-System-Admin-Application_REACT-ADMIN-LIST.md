...
Usage
Here is the minimal code necessary to display a list of posts using a <DataTable>:
...
You can find more advanced examples of <List> usage in the demos.
...
A <FilterButton> to display the filter form if you set the filters prop
...
You can also add custom actions, e.g. a <SelectColumnsButton> to let the user choose which columns to display in the list:
...
The aside prop is also the preferred way to add a Filter Sidebar to a list view:
...
The most common List child is <DataTable>:
...
<DataTable> displays records in a table
...
You can also render the list of records using a custom React component thanks to the render prop. Check Building a custom List Iterator for more details.
...
You can handle this case by getting the isPending variable from the useListContext hook:
...
Call the dataProvider with the current sort and filter (but without pagination),
Transform the result into a CSV string,
...
Tip: For CSV conversion, you can import jsonexport, a JSON to CSV converter which is already a react-admin dependency. And for CSV download, take advantage of react-admin’s downloadCSV function.
...
Tip: If you need to use RBAC to hide some columns based on user permissions, you can use useExporterWithAccessControl to apply access control to the exported records:
...
render
Alternatively to children, you can pass a render prop to <List>. It will receive the ListContext as its argument, and should return a React node.
...
You can learn more by reading the <ListGuesser> documentation.
...
Live Updates
If you want to subscribe to live updates on the list of records (topic: resource/[resource]), add the <ListLiveUpdate> component in your <List> children.
...
Headless Version
Besides fetching a list of records from the data provider, <List> renders the default list page layout (title, buttons, filters, a Material-UI <Card>, pagination) and its children. If you need a custom list layout, you may prefer the <ListBase> component, which only renders its children in a ListContext.
...
Anonymous Access
The <List> component requires authentication and will redirect anonymous users to the login page. If you want to allow anonymous access, use the disableAuthentication prop.
