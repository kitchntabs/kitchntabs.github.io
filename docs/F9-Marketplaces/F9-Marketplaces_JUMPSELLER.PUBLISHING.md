...
When publishing products to Jumpseller, the system automatically ensures that product categories are synchronized. Products maintain their original categories by creating matching categories in Jumpseller.

### Category Flow Architecture

```
Product Publishing Request
    ↓
parseProduct() - Prepare product data for Jumpseller
    ↓
getCategoryIds($product) - Get/Create category mappings
    ├── Check OutputCategoryMapping for existing mapping
    │   ├── Found: Return mapped Jumpseller category ID
    │   └── Not Found: Continue to create
    ↓
ensureCategoriesExist([$categoryName])
    ├── findCategoryByName() - Search in Jumpseller API
    │   ├── Found: Return existing category ID
    │   └── Not Found: Continue to create
    └── createCategory() - Create new category in Jumpseller
    ↓
Create SystemMarketplaceCategory (local mapping record)
    ↓
Create OutputCategoryMapping (product category → marketplace category)
    ↓
Return category_ids array for product
```

### Category Service Methods
**File:** `JumpsellerService.php` uses `CategoryServiceMethods` trait

#### Key Methods:

1. **getCategoryIds($product)** - Main entry point for category resolution
   - Checks for existing `OutputCategoryMapping`
   - Creates category in Jumpseller if not mapped
   - Falls back to default category if creation fails

2. **ensureCategoriesExist(array $categoryNames)** - Ensures categories exist in Jumpseller
   - Returns map of category names to Jumpseller category IDs
   - Creates categories that don't exist

3. **findCategoryByName(string $name)** - Searches for category in Jumpseller
   - API call: `GET /categories.json`
   - Case-insensitive name matching

4. **createCategory(string $name)** - Creates new category in Jumpseller
   - API call: `POST /categories.json`
   - Returns new category data with ID
...
OutputCategoryMapping
    ├── category_id (Product's category)
    ├── tenant_id (Product's tenant)
    └── system_marketplace_category_id
            ↓
SystemMarketplaceCategory
    ├── system_marketplace_id (Jumpseller marketplace)
    ├── source_id (Jumpseller category ID)
    ├── name (Category name)
    └── metadata (Full category data)
```

### Default Category

When a product doesn't have a category or category creation fails, the system uses a default category:

```php
public const DEFAULT_CATEGORY_SOURCE_ID = 'default_category';
```

### Category Integration in Product Publishing

Categories are included in the `parseProduct()` method:

```php
// In parseProduct()
$categoryIds = $this->getCategoryIds($product);
if (!empty($categoryIds)) {
    $parsedProduct['categories'] = array_map(function ($categoryId) {
...
## Category Synchronization Details

### How Categories Are Preserved

When products are published to Jumpseller, their original categories are preserved through automatic synchronization:

1. **First Check**: System looks for an existing `OutputCategoryMapping` that links the product's category to a Jumpseller category
2. **If Mapping Exists**: Uses the existing Jumpseller category ID
3. **If No Mapping**: 
   - Gets the product's category name from the local database
   - Searches Jumpseller for a category with the same name
   - If found, creates a mapping to the existing Jumpseller category
   - If not found, creates a new category in Jumpseller with the same name
   - Creates `SystemMarketplaceCategory` and `OutputCategoryMapping` records

### API Endpoints Used

| Operation | Endpoint | Description |
|-----------|----------|-------------|
| List Categories | `GET /categories.json` | Fetch all categories for name matching |
| Get Category | `GET /categories/{id}.json` | Verify category exists |
| Create Category | `POST /categories.json` | Create new category |

### Error Handling

- If category creation fails, product uses default category
- Category errors don't block product publishing
- Warnings are logged for failed category operations

### Best Practices

1. **Pre-synchronize Categories**: Run category sync before bulk publishing for better performance
2. **Consistent Naming**: Use consistent category names between your system and Jumpseller
3. **Default Category**: Ensure the default category exists in Jumpseller before publishing
